...
| `AGENT_ROOM_LOKET` | `loket.json` di folder repo | kuota loket per proyek: batas proses bersamaan per folder (`maksJalanProyek`) dan batas antre per folder (`maksAntreProyek`). Berkas tidak ada = fitur diam total dan antreannya FIFO seperti biasa. Contohnya `loket.contoh.json`; lihat [docs/05](05-kendali-web.md#kuota-loket-per-proyek-loketjson) |
...
| `AGENT_ROOM_LAGU` | *(kosong)* | berkas lagu kantor yang disetel Senin–Jumat jam 10 — **hanya kalau** centang 🎵 di panel ⚙️ dinyalakan (mati bawaan). Kosong = dicari sendiri sebagai `lagu-kantor.m4a`/`.mp3`/`.ogg`/`.opus`/`.webm`/`.wav` di folder proyek. Tidak ada berkasnya = jadwalnya cuma diam (lihat [Isi transkrip → suara](03-isi-transkrip.md)) |
